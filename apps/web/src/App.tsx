...
function LoadingScreen() {
  return (
    <main className="auth-shell">
      <Logo size="large" />
      <p className="loading-label">Запускаем пространство…</p>
    </main>
  );
}
...
function BootstrapScreen({ api, error, onError, onAuthenticated }: AuthProps) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      onAuthenticated(
        (await api.bootstrap(input, stringValue(values, "bootstrap_token")))
          .user,
      );
    } catch (cause) {
      onError(messageOf(cause));
  ...
  return (
    <AuthLayout
      ...
      lead="Первый аккаунт получит права владельца"
    >
      <form className="auth-form auth-form--grid" onSubmit={submit}>
        <div className="form-span">
          <Field
            label="Код первичной настройки"
            hint="В production — значение BOOTSTRAP_TOKEN из .env сервера"
...
function InviteScreen({ api, error, onError, onAuthenticated }: AuthProps) {
  ...
  return (
    <AuthLayout
      ...
      lead="Заполните профиль рабочего аккаунта"
    >
      <form className="auth-form" onSubmit={submit}>
        <Field
          label="Ваше имя"
        ...
        <FormError message={error} />
        <Button type="submit" variant="primary" disabled={pending}>
          {pending ? "Присоединяем…" : "Присоединиться"}
        </Button>
      </form>
    </AuthLayout>
  );
...
function AuthLayout({
  ...
  return (
    <main className="auth-shell">
      ...
      <small className="auth-footer">Open source · Self-hosted</small>
    </main>
  );
}
...
function Logo({ size = "normal" }: { size?: "normal" | "large" }) {
  return (
    <div className={cx("brand", size === "large" && "brand--large")}>
      <img className="brand__mark" src={comaLogo} alt="" />
      <strong>Coma</strong>
    </div>
  );
}
...
function MessengerScreen({
  ...
  async function openDirectory() {
    ...
    setDialog("discover");
    try {
      setDirectory(await api.discoverChats());
    } catch (cause) {
      setError(messageOf(cause));
    }
  }
    ...
    <main className="messenger-shell">
      <aside className="sidebar">
        ...
        <div className="search-actions">
          <Button className="search-button" size="sm">
            <Search size={17} />
            <span>Найти</span>
            <kbd>⌘ K</kbd>
          </Button>
        ...
        <div className="chat-scroll">
          ...
          <ChatFolder title="Личные сообщения" count={directChats.length}>
            ...
            {!directChats.length && (
              <div className="folder-hint">Пока нет диалогов</div>
            )}
          </ChatFolder>
        </div>
      ...
      <section className="conversation">
        {selected ? (
          ...
          <WorkspaceEmpty onCreate={() => setDialog("create")} />
        )}
      </section>
      ...
      {dialog === "create" && (
        <CreateChatDialog
          api={api}
          canCreateChannel={user.role !== "member"}
      ...
      {dialog === "discover" && (
        <DirectoryDialog
          ...
          onClose={() => setDialog(null)}
          onJoin={async (id) => {
            const chat = await api.joinChat(id);
            setChats((items) => [chat, ...items]);
            setSelectedID(chat.id);
            setDirectory((items) => items.filter((item) => item.id !== id));
          }}
        />
      )}
    </main>
...
function SidebarShortcut({
  ...
  return (
    <button
      ...
      disabled
      title={`${label} появится в следующих фазах`}
    >
      {icon}
      <span>{label}</span>
    </button>
...
function ChatFolder({
  ...
  return (
    <section className="chat-folder">
      <div className="chat-folder__head">
        <button>
          <ChevronDown size={14} />
          {title}
        </button>
        <span>{count || ""}</span>
        {action && (
          <button className="chat-folder__action" onClick={action}>
            {actionLabel}
          </button>
        )}
      </div>
      <div className="chat-folder__items">{children}</div>
    </section>
  );
}
...
function ChatRow({
  ...
  return (
    <button
      className={cx("chat-row", active && "chat-row--active")}
      onClick={onClick}
    >
      <ChatGlyph kind={chat.kind} />
      <span>{chatTitle(chat)}</span>
      {chat.visibility === "private" && chat.kind !== "direct" && (
        <Lock size={12} />
      )}
    </button>
  );
  ...
  return (
    <>
      <header className="conversation-header">
        <span className="conversation-header__glyph">
          <ChatGlyph kind={chat.kind} size={18} />
        </span>
        ...
        <div className="conversation-header__actions">
          <IconButton label="Поиск по чату">
            <Search size={18} />
          </IconButton>
          <IconButton label="Участники">
            <Users size={18} />
          </IconButton>
          <IconButton label="Информация о чате">
            <Info size={18} />
          </IconButton>
      ...
      <div className="conversation-body">
        <div className="chat-start">
          <span>
            <ChatGlyph kind={chat.kind} size={28} />
          </span>
          <h1>{chatTitle(chat)}</h1>
          <p>
            {chat.kind === "channel"
              ? "Здесь публикуют объявления владельцы и администраторы."
              : "Это начало чата. Сообщения появятся в следующем инкременте."}
          </p>
        </div>
        <article className="message-row">
          ...
          <div className="message-row__content">
            ...
            <p>
              Чат создан. Здесь появятся сообщения команды, ответы в тредах и
              реакции.
            </p>
            <button className="thread-chip" disabled>
              <MessageCircle size={15} />
              <span>Комментарии</span>
            </button>
          </div>
          <div className="message-actions">
            <IconButton label="Реакция">
              <Smile size={16} />
            </IconButton>
            <IconButton label="Обсудить">
      ...
      <div className="composer-wrap">
        <div className="composer">
          <div className="composer__input">Сообщение для {chatTitle(chat)}</div>
          <div className="composer__toolbar">
            <div>
              <IconButton label="Прикрепить файл" disabled>
                <Paperclip size={18} />
              </IconButton>
              <IconButton label="Упомянуть" disabled>
                <AtSign size={18} />
              </IconButton>
              <IconButton label="Эмодзи" disabled>
                <Smile size={18} />
              </IconButton>
            </div>
            <IconButton className="composer-send" label="Отправить" disabled>
              <Send size={18} />
            </IconButton>
          </div>
        </div>
...
function WorkspaceEmpty({ onCreate }: { onCreate: () => void }) {
  return (
    <div className="workspace-empty">
      ...
      <h1>Создайте первый чат</h1>
      <p>Обсуждайте работу в чатах или публикуйте новости в каналах.</p>
      <div>
...
function CreateChatDialog({
    ...
    try {
      onCreated(
        await api.createChat({
          kind,
          visibility: stringValue(values, "visibility") as "private" | "public",
          name: stringValue(values, "name"),
          topic: stringValue(values, "topic"),
        }),
      );
    } catch (cause) {
      setError(messageOf(cause));
  ...
  return (
    <Dialog
      title="Создать чат"
      description="Выберите формат общения и доступ"
      onClose={onClose}
    >
      <div className="type-switch">
        <button
          className={cx(kind === "group" && "active")}
          onClick={() => setKind("group")}
        >
          <MessagesSquare size={20} />
          <span>
            <strong>Чат</strong>
            <small>Все участники могут писать</small>
          </span>
        </button>
        <button
          className={cx(kind === "channel" && "active")}
        ...
        >
          ...
          <span>
            <strong>Канал</strong>
            <small>Пишут только администраторы</small>
          </span>
        </button>
      </div>
      <form className="dialog-form" onSubmit={submit}>
        <Field
          ...
          maxLength={120}
          placeholder={
            kind === "group" ? "Команда продукта" : "Важные объявления"
          }
        />
        <TextareaField
          label="Описание"
          name="topic"
          maxLength={500}
          placeholder="О чём здесь говорят"
        />
        <SelectField label="Доступ" name="visibility">
          <option value="private">Закрытый — по приглашению</option>
          <option value="public">Открытый — можно найти и вступить</option>
        </SelectField>
        <FormError message={error} />
...
function DirectoryDialog({
  ...
  return (
    <Dialog
      title="Обзор чатов"
      description="Открытые чаты и каналы пространства"
      onClose={onClose}
    >
      <div className="directory-search">
        <Search size={17} />
        <span>Поиск по открытым чатам</span>
      </div>
      <div className="directory-list">
        {chats.map((chat) => (
          <div className="directory-row" key={chat.id}>
            ...
            <div>
              ...
              <span>
                {chat.topic || (chat.kind === "channel" ? "Канал" : "Чат")}
              </span>
            </div>
            <Button
              size="sm"
              disabled={joining === chat.id}
              onClick={async () => {
                setJoining(chat.id);
                setLocalError("");
                try {
                  await onJoin(chat.id);
                } catch (cause) {
        ...
        {!chats.length && !error && (
          <div className="dialog-empty">
            <Compass size={24} />
            <span>Новых открытых чатов пока нет</span>
          </div>
        )}
      </div>
