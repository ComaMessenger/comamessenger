...
func (h *identityHandlers) writeAgentToolError(w standardhttp.ResponseWriter, r *standardhttp.Request, err error) {
	...
	case errors.Is(err, agenttool.ErrForbidden), errors.Is(err, agenttool.ErrConfirmationRequired), errors.Is(err, message.ErrForbidden), errors.Is(err, files.ErrForbidden), errors.Is(err, chat.ErrForbidden):
