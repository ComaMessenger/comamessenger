...
- Персональные read receipts не хранятся как отдельные доставки. По явному запросу участника Core вычисляет список прочитавших из монотонных `chat_reads`/`thread_reads`, показывает только текущих участников и исключает автора сообщения. Это on-demand представление, а не дополнительный источник истины.
