...
3. **Один бинарник + Postgres.** Никаких Elasticsearch, Kafka, RabbitMQ в базовой поставке. Redis опционален (см. раздел 2.6).
...
- [ ] Модуль `authz`
- [ ] Регистрация, логин, refresh, сессии, приглашения
- [ ] Организация, актор, профиль
- [ ] Чаты и каналы: CRUD, membership, видимость и серверная проверка права публикации
...
- [ ] Auth-экраны, онбординг организации
...
## 10. Вопросы, которые нужно закрыть до старта кода

1. Go для ядра окончательно, или сначала Node для быстрого MVP с миграцией позже.
2. Redis обязателен или опционален (влияет на дизайн pub/sub и кэша unread).
3. Markdown-подмножество: какое именно (CommonMark без таблиц? mentions-синтаксис?).
4. Expo или bare React Native.
5. Какие три встроенных агента реально показывают ценность именно твоей целевой аудитории.
6. Первые пилотные команды: кто и когда.
