...
`min_retained_seq` — минимальный checkpoint, с которого сервер ещё может полностью продолжить поток. Если первое сохранённое событие имеет `seq=1201`, значение поля равно `1200`; поэтому новый журнал с первым событием `seq=1` корректно принимает `last_seq=0`.
